...
GUI::Window Strip::CreateButton(const char *text, size_t ident, bool check) {
	...
	SetWindowTheme(HwndOf(w), L"DarkMode_Explorer", NULL); // for BS_PUSHBUTTON
	...
	SetWindowTheme(HwndOf(wToolTip), L"DarkMode_Explorer", NULL);
...
void Strip::Paint(HDC hDC) {
	...
	::FillRect(hDC, &rc, BG_BRUSH_DARK); // hbrFace);
...
//		} else {
			...
			DrawFrameControl(hDC, &rcClose, DFC_CAPTION,
					 DFCS_CAPTIONCLOSE | closeAppearence  | DFCS_FLAT);
		//}
	}
}
...
LRESULT Strip::CustomDraw(NMHDR *pnmh) noexcept {
	...
	if ((pcd->dwDrawStage == CDDS_PREERASE) || (pcd->dwDrawStage == CDDS_PREPAINT)) {
		...
		// draw border
		FillRect(pcd->hdc, &rcButton, CONTROL_BORDER_BRUSH_DARK);
		InflateRect(&rcButton, -1, -1);

		// fill
		if (checked || pcd->uItemState & CDIS_SELECTED)
			FillRect(pcd->hdc, &rcButton, CONTROL_BG_BRUSH_SELECTED_DARK);
		else if (pcd->uItemState & CDIS_HOT)
			FillRect(pcd->hdc, &rcButton, CONTROL_BG_BRUSH_HOT_DARK);
		else
			FillRect(pcd->hdc, &rcButton, CONTROL_BG_BRUSH_DARK);
		...
		constexpr DWORD colourTransparent = RGB(0x3F, 0x3F, 0x3F); // RGB(0xC0, 0xC0, 0xC0)

		// Offset from button edge to contents.
		const int xOffset = ((rcButton.right - rcButton.left) - rbmi.bmiHeader.biWidth) / 2 + 1;
		const int yOffset = ((rcButton.bottom - rcButton.top) - rbmi.bmiHeader.biHeight) / 2   +1;
...
void FindStrip::Creation() {
	...
	SetWindowTheme(HwndOf(wText), L"DarkMode_CFD", NULL);
	SetFontHandle(wText, fontText);
	wText.Show();
...
void FindStrip::Size() {
	...
	const Interval verticalCheck(rcArea.top, rcArea.bottom -1);
...
void ReplaceStrip::Creation() {
	...
	SetWindowTheme(HwndOf(wReplace), L"DarkMode_CFD", NULL);
...
void ReplaceStrip::Size() {
	...
	Interval verticalCheck(rcLine.top, rcLine.bottom - 1);
	...
	verticalCheck = Interval(rcLine.top, rcLine.bottom -1);
